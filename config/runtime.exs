...
#
#     PHX_SERVER=true bin/lobby start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :lobby, LobbyWeb.Endpoint, server: true
end

config :lobby, LobbyWeb.Endpoint, http: [port: String.to_integer(System.get_env("PORT", "4000"))]
...
if config_env() == :prod do
  ...
  config :lobby, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  cors_origins =
    case System.get_env("CORS_ORIGINS") do
      nil -> ["https://tauri.localhost"]
      origins -> String.split(origins, ",", trim: true)
    end

  config :lobby, :cors_origins, cors_origins

  # Guardian JWT secret for production
  guardian_secret =
